...
// Servo position
int servo2Position = 0; // Initial position of servo2

// Last scan time
unsigned long lastScanTime = 0;
const unsigned long scanInterval = 4500; // 5 seconds
...
void setup() {
  ...
  servo2.write(servo2Position); // Set initial position to 0 degrees
...
void loop() {
  ...
  if (currentTime - lastScanTime >= scanInterval) {
    ...
    if (red > 10 && green < 3 && blue < 3) {
      redCounter++;
      updated = true;
      servo2Position = 0; // Move to 30° for red
    } else if (green > 10 && red < 3 && blue < 3) {
      greenCounter++;
      updated = true;
      servo2Position = 30; // Move to 60° for green
    } else if (blue > 10 && green < 6 && red < 3) {
      blueCounter++;
      updated = true;
      servo2Position = 60; // Move to 90° for blue
    } else if (red > 3 && green > 6) {
      yellowCounter++;
      updated = true;
      servo2Position = 90; // Move to 120° for yellow
    }

    // Update the servo position if there was a color change
    if (updated) {
